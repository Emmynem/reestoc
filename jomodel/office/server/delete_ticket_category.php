<?php
  ...
  class deleteTicketCategoryClass {
  ...
  if ($connected) {
    try {
      ...
      $sql = "SELECT * FROM $table WHERE event_unique_id=:event_unique_id AND unique_id=:unique_id";
      ...
      if ($query->rowCount() > 0) {
        $sql2 = "UPDATE $table SET edit_user_unique_id=:edit_user_unique_id, status=:status, last_modified=:last_modified WHERE event_unique_id=:event_unique_id AND unique_id=:unique_id";
        ...
        if ($query2) {
          $returnvalue = new deleteTicketCategoryClass();
