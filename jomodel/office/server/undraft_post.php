<?php
  ...
  if ($connected) {
    try {
      ...
      if ($query->rowCount() > 0) {
        $sql2 = "UPDATE $table SET edit_user_unique_id=:edit_user_unique_id, drafted=:drafted, added_date=:added_date, last_modified=:last_modified WHERE unique_id=:unique_id";
        $query2 = $conn->prepare($sql2);
        $query2->bindParam(':edit_user_unique_id', $data['edit_user_unique_id']);
        $query2->bindParam(':unique_id', $data['unique_id']);
        $query2->bindParam(':drafted', $data['drafted']);
        $query2->bindParam(':added_date', $date_added);
        $query2->bindParam(':last_modified', $date_added);
        $query2->execute();

        if ($query2) {
          $returnvalue = new undraftPostClass();
