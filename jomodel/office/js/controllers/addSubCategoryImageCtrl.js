xnyder.controller('add-sub-category-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Add Sub Category Image";
  ...
  $scope.get_sub_category_details = function (sub_category_unique_id, stripped) {

    $scope.genericData = {
      "sub_category_unique_id":sub_category_unique_id ? sub_category_unique_id : null,
      "stripped":stripped ? stripped : null
    }

    $http.post('../../ng/server/get/get_sub_category_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          $scope.subCategoryDetailsStatus = undefined;
          $scope.subCategoryDetails = response.data.resultData;
          $scope.sub_category_name = $scope.subCategoryDetails.name;

        }
        else if (response.data.engineError == 2) {
          $scope.subCategoryDetailsStatus = response.data.engineErrorMessage;
          // $timeout(function () {
          //   $scope.subCategoryDetailsStatus = undefined;
          // }, 3000)
        }
        else {
          $scope.subCategoryDetailsStatus = "Error Occured";
          // $timeout(function () {
          //   $scope.subCategoryDetailsStatus = undefined;
          // }, 3000)
        }
      }, function (error) {
        $scope.subCategoryDetailsStatus = "Something's Wrong";
        // $timeout(function () {
        //   $scope.subCategoryDetailsStatus = undefined;
        // }, 3000)
      })
  ...
  $scope.get_sub_category_details($scope.the_sub_category_unique_id, "");

  $scope.upload_sub_category_images = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 1) {
        ...
        $http.post('../../ng/server/data/sub_category_images/add_sub_category_image_select_type.php', uploadForm, {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Add Activity", "Sub Category Image added");
