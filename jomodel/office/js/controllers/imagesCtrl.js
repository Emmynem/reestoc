xnyder.controller('imagesCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$window,storage,notify,ngClipboard){

  $rootScope.pageTitle = "Blog Images";
  ...
  $scope.filter = function () {
    ...
    if ($scope.filter_var.startdate > $scope.filter_var.enddate) {
      ...
      $http.post('server/filter_images.php', $scope.filter_data)
        .then(function (response) {
          if (response.data.engineMessage == 1) {
            $scope.if_images_exists = undefined;
            ...
            $scope.if_images_exists = "No data in range !";
            $scope.remove_loader();
            // $timeout(function () {
            //   $scope.if_images_exists = undefined;
            // }, 3000)
            ...
            $scope.if_images_exists = "Couldn't get data !";
            $scope.remove_loader();
            // $timeout(function () {
            //   $scope.if_images_exists = undefined;
            // }, 3000)
          ...
          $scope.if_images_exists = "Something's Wrong!";
          $scope.remove_loader();
          // $timeout(function () {
          //   $scope.if_images_exists = undefined;
          // }, 3000)
  ...
  $scope.load = function () {
    ...
    $http.get('server/get_images.php')
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          $scope.if_images_exists = undefined;
          ...
          $scope.if_images_exists = "No images found !";
          $scope.remove_loader();
          // $timeout(function () {
          //   $scope.if_images_exists = undefined;
          // }, 3000)
          ...
          $scope.if_images_exists = "Couldn't fetch images !";
          $scope.remove_loader();
          // $timeout(function () {
          //   $scope.if_images_exists = undefined;
          // }, 3000)
    ...
    if ($scope.count_files > 20) {
      ...
      $scope.continue_action = function () {
        ...
        if ($scope.count_files > 20) {
          ...
          //     uploadForm.append('file[]', file);
          // });
          $http.post('server/uploadimages.php', uploadForm, {
          ...
          .then(function(response){
              ...
              if(response.data.error){
                ...
                $scope.notification_message = "Blog " + $rootScope.fileUploadStatusSuccess;
  ...
  $scope.deleteImage = function (unique_id) {
    ...
    $scope.continue_action = function () {
      ...
      $http.post('server/delete_blog_image.php', $scope.deleteData)
        .then(function (response) {
          if (response.data.engineMessage == 1) {
            $scope.showSuccessStatus = "Image Deleted";
            notify.do_notify($scope.result_u, "Delete Activity", "Blog image deleted");
            $timeout(function () {
              $scope.showStatus = undefined;
              $scope.showSuccessStatus = undefined;
