xnyder.controller('edit-mini-category-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Edit Mini Category Image";
  ...
  $scope.the_mini_category_unique_id = $routeParams.mini_category_unique_id;
  $scope.the_mini_category_image_unique_id = $routeParams.mini_category_image_unique_id;

  $scope.mini_category_images = {};
  ...
  $scope.get_mini_category_image_details = function (unique_id, mini_category_unique_id) {
    ...
    $http.post('../../ng/server/get/get_mini_category_image_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          $scope.miniCategoryImageDetails = null;
          $scope.miniCategoryImageDetailsStatus = undefined;
          $scope.miniCategoryImageDetails = response.data.resultData;
          $scope.mini_category_name = $scope.miniCategoryImageDetails[0].mini_category_name;
          $scope.mini_category_file = $scope.miniCategoryImageDetails[0].file;
        }
        else if (response.data.engineError == 2) {
          $scope.miniCategoryImageDetailsStatus = response.data.engineErrorMessage;
          // $timeout(function () {
          //   $scope.miniCategoryImageDetailsStatus = undefined;
          // }, 3000)
        }
        else {
          $scope.miniCategoryImageDetailsStatus = "Error Occured";
          // $timeout(function () {
          //   $scope.miniCategoryImageDetailsStatus = undefined;
          // }, 3000)
        }
      }, function (error) {
        $scope.miniCategoryImageDetailsStatus = "Something's Wrong";
        // $timeout(function () {
        //   $scope.miniCategoryImageDetailsStatus = undefined;
        // }, 3000)
      })
  ...
  $scope.get_mini_category_image_details($scope.the_mini_category_image_unique_id, $scope.the_mini_category_unique_id);

  $scope.edit_mini_category_image = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 1) {
        ...
        uploadForm.append('upload_type', $scope.mini_category_images.upload_type);
        uploadForm.append('mini_category_unique_id', $scope.the_mini_category_unique_id);
        uploadForm.append('mini_category_image_unique_id', $scope.the_mini_category_image_unique_id);
        angular.forEach($scope.filez, function(file){
            uploadForm.append('file[]', file);
        });
        $http.post('../../ng/server/data/mini_category_images/edit_mini_category_image_select_type.php', uploadForm, {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Edit Activity", "Mini Category Image edited");
            ...
            $timeout(function () {
              $location.path("product-mini-categories");
            }, 1000);
