xnyder.controller('add-product-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Add Product Image";
  ...
  $scope.get_product_details = function (product_unique_id, stripped) {
    ...
    $http.post('../../ng/server/get/get_product_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          ...
          $scope.product_name = $scope.productDetails[0].name;
  ...
  $scope.get_product_details($scope.the_product_unique_id, "");
      ...
      if ($scope.count_files > 10) {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Add Activity", "Product Image added");
