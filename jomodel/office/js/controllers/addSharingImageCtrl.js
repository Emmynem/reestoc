xnyder.controller('add-sharing-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Add Sharing Item Image";
  ...
  $scope.get_sharing_details = function (sharing_unique_id, stripped) {
  ...
  $scope.get_sharing_details($scope.the_sharing_unique_id, "");

  $scope.upload_sharing_images = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 10) {
        ...
        $http.post('../../ng/server/data/sharing_images/add_sharing_image_select_type.php', uploadForm, {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Add Activity", "Sharing Item Image added");
