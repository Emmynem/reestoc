xnyder.controller('edit-brand-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Edit Brand Image";
  ...
  $scope.the_brand_image_unique_id = $routeParams.brand_image_unique_id;
  ...
  $scope.get_brand_image_details = function (unique_id, brand_unique_id) {
    ...
    $http.post('../../ng/server/get/get_brand_image_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          $scope.brandImageDetails = null;
          $scope.brandImageDetailsStatus = undefined;
          $scope.brandImageDetails = response.data.resultData;
          $scope.brand_name = $scope.brandImageDetails[0].brand_name;
          $scope.brand_file = $scope.brandImageDetails[0].file;
        }
        else if (response.data.engineError == 2) {
          $scope.brandImageDetailsStatus = response.data.engineErrorMessage;
          // $timeout(function () {
          //   $scope.brandImageDetailsStatus = undefined;
          // }, 3000)
        }
        else {
          $scope.brandImageDetailsStatus = "Error Occured";
          // $timeout(function () {
          //   $scope.brandImageDetailsStatus = undefined;
          // }, 3000)
        }
      }, function (error) {
        $scope.brandImageDetailsStatus = "Something's Wrong";
        // $timeout(function () {
        //   $scope.brandImageDetailsStatus = undefined;
        // }, 3000)
      })
  ...
  $scope.get_brand_image_details($scope.the_brand_image_unique_id, $scope.the_brand_unique_id);

  $scope.edit_brand_image = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 1) {
        ...
        uploadForm.append('brand_image_unique_id', $scope.the_brand_image_unique_id);
        angular.forEach($scope.filez, function(file){
            uploadForm.append('file[]', file);
        });
        $http.post('../../ng/server/data/brand_images/edit_brand_image_select_type.php', uploadForm, {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Edit Activity", "Brand Image edited");
