xnyder.controller('add-mini-category-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Add Mini Category Image";
  ...
  $scope.get_mini_category_details = function (mini_category_unique_id, stripped) {

    $scope.genericData = {
      "mini_category_unique_id":mini_category_unique_id ? mini_category_unique_id : null,
      "stripped":stripped ? stripped : null
    }

    $http.post('../../ng/server/get/get_mini_category_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          $scope.miniCategoryDetailsStatus = undefined;
          $scope.miniCategoryDetails = response.data.resultData;
          $scope.mini_category_name = $scope.miniCategoryDetails.name;

        }
        else if (response.data.engineError == 2) {
          $scope.miniCategoryDetailsStatus = response.data.engineErrorMessage;
          // $timeout(function () {
          //   $scope.miniCategoryDetailsStatus = undefined;
          // }, 3000)
        }
        else {
          $scope.miniCategoryDetailsStatus = "Error Occured";
          // $timeout(function () {
          //   $scope.miniCategoryDetailsStatus = undefined;
          // }, 3000)
        }
      }, function (error) {
        $scope.miniCategoryDetailsStatus = "Something's Wrong";
        // $timeout(function () {
        //   $scope.miniCategoryDetailsStatus = undefined;
        // }, 3000)
      })
  ...
  $scope.get_mini_category_details($scope.the_mini_category_unique_id, "");

  $scope.upload_mini_category_images = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 1) {
        ...
        $http.post('../../ng/server/data/mini_category_images/add_mini_category_image_select_type.php', uploadForm, {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Add Activity", "Mini Category Image added");
