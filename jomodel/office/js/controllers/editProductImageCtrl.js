xnyder.controller('edit-product-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Edit Product Image";
  ...
  $scope.the_product_unique_id = $routeParams.product_unique_id;
  $scope.the_product_image_unique_id = $routeParams.product_image_unique_id;
  ...
  $scope.get_product_image_details = function (unique_id, product_unique_id) {
    ...
    $http.post('../../ng/server/get/get_product_image_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          $scope.productImageDetails = null;
          $scope.productImageDetailsStatus = undefined;
          $scope.productImageDetails = response.data.resultData;
          $scope.product_name = $scope.productImageDetails[0].product_name;
          $scope.product_file = $scope.productImageDetails[0].file;
        }
        else if (response.data.engineError == 2) {
          $scope.productImageDetailsStatus = response.data.engineErrorMessage;
          // $timeout(function () {
          //   $scope.productImageDetailsStatus = undefined;
          // }, 3000)
        }
        else {
          $scope.productImageDetailsStatus = "Error Occured";
          // $timeout(function () {
          //   $scope.productImageDetailsStatus = undefined;
          // }, 3000)
        }
      }, function (error) {
        $scope.productImageDetailsStatus = "Something's Wrong";
        // $timeout(function () {
        //   $scope.productImageDetailsStatus = undefined;
        // }, 3000)
      })
  ...
  $scope.get_product_image_details($scope.the_product_image_unique_id, $scope.the_product_unique_id);

  $scope.edit_product_image = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 1) {
        ...
        uploadForm.append('product_unique_id', $scope.the_product_unique_id);
        uploadForm.append('product_image_unique_id', $scope.the_product_image_unique_id);
