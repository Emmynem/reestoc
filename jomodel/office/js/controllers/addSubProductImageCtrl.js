xnyder.controller('add-sub-product-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Add Sub Product Image";
  ...
  $scope.get_sub_product_details = function (sub_product_unique_id, stripped) {
    ...
    $http.post('../../ng/server/get/get_sub_product_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          ...
          $scope.sub_product_name = $scope.subProductDetails[0].product_name + " | " + $scope.subProductDetails[0].name;
  ...
  $scope.get_sub_product_details($scope.the_sub_product_unique_id, "");

  $scope.upload_sub_product_images = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 10) {
        ...
        $http.post('../../ng/server/data/sub_product_images/add_sub_product_image_select_type.php', uploadForm, {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Add Activity", "Sub Product Image added");
