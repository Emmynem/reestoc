xnyder.controller('edit-sharing-imageCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$routeParams,$window,storage,notify){

  $rootScope.pageTitle = "Edit Sharing Item Image";
  ...
  $scope.the_sharing_unique_id = $routeParams.sharing_unique_id;
  $scope.the_sharing_image_unique_id = $routeParams.sharing_image_unique_id;
  ...
  $scope.get_sharing_image_details = function (unique_id, sharing_unique_id) {
    ...
    $http.post('../../ng/server/get/get_sharing_image_details.php', $scope.genericData)
      .then(function (response) {
        if (response.data.engineMessage == 1) {
          $scope.sharingImageDetails = null;
          $scope.sharingImageDetailsStatus = undefined;
          $scope.sharingImageDetails = response.data.resultData;
          $scope.sharing_name = $scope.sharingImageDetails[0].sharing_name;
          $scope.sharing_file = $scope.sharingImageDetails[0].file;
        }
        else if (response.data.engineError == 2) {
          $scope.sharingImageDetailsStatus = response.data.engineErrorMessage;
          // $timeout(function () {
          //   $scope.sharingImageDetailsStatus = undefined;
          // }, 3000)
        }
        else {
          $scope.sharingImageDetailsStatus = "Error Occured";
          // $timeout(function () {
          //   $scope.sharingImageDetailsStatus = undefined;
          // }, 3000)
        }
      }, function (error) {
        $scope.sharingImageDetailsStatus = "Something's Wrong";
        // $timeout(function () {
        //   $scope.sharingImageDetailsStatus = undefined;
        // }, 3000)
      })
  ...
  $scope.get_sharing_image_details($scope.the_sharing_image_unique_id, $scope.the_sharing_unique_id);

  $scope.edit_sharing_image = function(){
    ...
    $scope.continue_action = function () {
      if ($scope.count_files > 1) {
        ...
        uploadForm.append('upload_type', $scope.sharing_images.upload_type);
        uploadForm.append('sharing_unique_id', $scope.the_sharing_unique_id);
        uploadForm.append('sharing_image_unique_id', $scope.the_sharing_image_unique_id);
        angular.forEach($scope.filez, function(file){
            uploadForm.append('file[]', file);
        });
        $http.post('../../ng/server/data/sharing_images/edit_sharing_image_select_type.php', uploadForm, {
        ...
        .then(function(response){
          if (response.data.engineMessage == 1) {
            ...
            notify.do_notify($scope.result_u, "Edit Activity", "Sharing Item Image edited");
