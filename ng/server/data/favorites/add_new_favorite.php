  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql2 = "SELECT unique_id FROM favorites WHERE user_unique_id=:user_unique_id AND product_unique_id=:product_unique_id AND status=:status";
        ...
        if ($query2->rowCount() > 0) {
          ...
          $returnvalue->engineErrorMessage = "Favorite exists";
        }
        else {
          $unique_id = $functions->random_str(20);

          $sql = "INSERT INTO favorites (unique_id, user_unique_id, product_unique_id, added_date, last_modified, status)
          ...
          if ($query->rowCount() > 0) {
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Not inserted (new favorite)";
          }
        }
