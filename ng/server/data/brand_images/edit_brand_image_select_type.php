<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      // $data['brand_image_unique_id'] = $_POST['brand_image_unique_id'];
      // $data['user_unique_id'] = $_POST['user_unique_id'];
      // $data['brand_unique_id'] = $_POST['brand_unique_id'];
      // $data['upload_type'] = $_POST['upload_type'];

      $brand_image_unique_id = isset($_POST['brand_image_unique_id']) ? $_POST['brand_image_unique_id'] : $data['brand_image_unique_id'];
      $user_unique_id = isset($_POST['user_unique_id']) ? $_POST['user_unique_id'] : $data['user_unique_id'];
      $brand_unique_id = isset($_POST['brand_unique_id']) ? $_POST['brand_unique_id'] : $data['brand_unique_id'];
      $upload_type = isset($_POST['upload_type']) ? $_POST['upload_type'] : $data['upload_type'];

      $path_to_upload = "../../../../images/brand_images/";
      // $path_to_delete = $_SERVER['DOCUMENT_ROOT']."/images/brand_images"; // For online own
      $path_to_delete = $_SERVER['DOCUMENT_ROOT']."/cerotics_store/images/brand_images"; // For offline own
      // $path_to_save = "https://www.reestoc.com/images/brand_images/";
      $path_to_save = "https://www.reestoc.com/images/brand_images/";
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql2 = "SELECT unique_id FROM brands WHERE unique_id=:unique_id AND status=:status";
        $query2 = $conn->prepare($sql2);
        $query2->bindParam(":unique_id", $brand_unique_id);
        $query2->bindParam(":status", $active);
        $query2->execute();

        if ($query2->rowCount() > 0) {

          $sql3 = "SELECT file FROM brand_images WHERE unique_id=:unique_id AND brand_unique_id=:brand_unique_id AND status=:status";
          $query3 = $conn->prepare($sql3);
          $query3->bindParam(":unique_id", $brand_image_unique_id);
          $query3->bindParam(":brand_unique_id", $brand_unique_id);
          ...
          if (empty($_FILES['file']['name'])) {
            ...
            $the_brand_image_details = $query3->fetch();
            $old_brand_image = $the_brand_image_details[0];
                              ...
                              if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                ...
                                $targetPath = $path; // Target path where file is to be stored
                                 ...
                                 $file = $path;
                                  ...
                                  $sql = "UPDATE brand_images SET edit_user_unique_id=:edit_user_unique_id, image=:image, file=:file, file_size=:file_size, last_modified=:last_modified
                                  WHERE unique_id=:unique_id AND brand_unique_id=:brand_unique_id";
                                  $query = $conn->prepare($sql);
                                  $query->bindParam(":unique_id", $brand_image_unique_id);
                                  ...
                                  if($query){
                                    if($count > 1){
                                      unlink($path_to_delete."/".$old_brand_image);
                                      $returnvalue = new genericClass();
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Old Brand image not found";
          }
          ...
          $returnvalue = new genericClass();
          $returnvalue->engineError = 2;
          $returnvalue->engineErrorMessage = "Brand not found";
        }
