    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql3 = "SELECT unique_id FROM brand_images WHERE brand_unique_id=:brand_unique_id AND status=:status";
          ...
          if (empty($_FILES['file']['name'])) {
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Not more than 1 files allowed";
          }
          else if ($query3->rowCount() >= $image_count) {
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Brand images maxed out";
          }
                              ...
                              if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                ...
                                $targetPath = $path; // Target path where file is to be stored
                                 ...
                                 $file = $path;
                                  ...
                                  $sql = "INSERT INTO brand_images (unique_id, user_unique_id, edit_user_unique_id, brand_unique_id, image, file, file_size, added_date, last_modified, status)
                                  VALUES (:unique_id, :user_unique_id, :edit_user_unique_id, :brand_unique_id, :image, :file, :file_size, :added_date, :last_modified, :status)";
