<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $sqlSearchUser = "SELECT unique_id FROM riders WHERE unique_id=:unique_id AND status=:status";
      $querySearchUser = $conn->prepare($sqlSearchUser);
      $querySearchUser->bindParam(":unique_id", $rider_unique_id);
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          if ($query3->rowCount() > 0) {
            ...
            if ($query2->rowCount() > 0) {
              ...
              $sql = "UPDATE shipments SET current_location=:current_location, longitude=:longitude, latitude=:latitude, last_modified=:last_modified WHERE shipment_unique_id=:shipment_unique_id";
        ...
        $returnvalue = new genericClass();
        $returnvalue->engineError = 2;
        $returnvalue->engineErrorMessage = "Rider not found";
      }
