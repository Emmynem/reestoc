<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      // $data['sub_category_image_unique_id'] = $_POST['sub_category_image_unique_id'];
      // $data['user_unique_id'] = $_POST['user_unique_id'];
      // $data['sub_category_unique_id'] = $_POST['sub_category_unique_id'];
      // $data['upload_type'] = $_POST['upload_type'];

      $sub_category_image_unique_id = isset($_POST['sub_category_image_unique_id']) ? $_POST['sub_category_image_unique_id'] : $data['sub_category_image_unique_id'];
      $user_unique_id = isset($_POST['user_unique_id']) ? $_POST['user_unique_id'] : $data['user_unique_id'];
      $sub_category_unique_id = isset($_POST['sub_category_unique_id']) ? $_POST['sub_category_unique_id'] : $data['sub_category_unique_id'];
      $upload_type = isset($_POST['upload_type']) ? $_POST['upload_type'] : $data['upload_type'];

      $path_to_upload = "../../../../images/sub_category_images/";
      // $path_to_delete = $_SERVER['DOCUMENT_ROOT']."/images/sub_category_images"; // For online own
      $path_to_delete = $_SERVER['DOCUMENT_ROOT']."/cerotics_store/images/sub_category_images"; // For offline own
      // $path_to_save = "https://www.reestoc.com/images/sub_category_images/";
      $path_to_save = "https://www.reestoc.com/images/sub_category_images/";
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql2 = "SELECT unique_id FROM sub_category WHERE unique_id=:unique_id AND status=:status";
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql3 = "SELECT file FROM sub_category_images WHERE unique_id=:unique_id AND sub_category_unique_id=:sub_category_unique_id AND status=:status";
          $query3 = $conn->prepare($sql3);
          $query3->bindParam(":unique_id", $sub_category_image_unique_id);
          ...
          if (empty($_FILES['file']['name'])) {
            ...
            $the_sub_category_image_details = $query3->fetch();
            $old_sub_category_image = $the_sub_category_image_details[0];
                              ...
                              if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                ...
                                $targetPath = $path; // Target path where file is to be stored
                                 ...
                                 $file = $path;
                                  ...
                                  $sql = "UPDATE sub_category_images SET edit_user_unique_id=:edit_user_unique_id, image=:image, file=:file, file_size=:file_size, last_modified=:last_modified
                                  WHERE unique_id=:unique_id AND sub_category_unique_id=:sub_category_unique_id";
                                  $query = $conn->prepare($sql);
                                  $query->bindParam(":unique_id", $sub_category_image_unique_id);
                                  ...
                                  if($query){
                                    if($count > 1){
                                      unlink($path_to_delete."/".$old_sub_category_image);
                                      $returnvalue = new genericClass();
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Old Sub Category image not found";
          }
