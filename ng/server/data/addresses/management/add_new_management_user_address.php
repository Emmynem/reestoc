    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          $sql2 = "SELECT address FROM management_addresses WHERE user_unique_id=:user_unique_id AND status=:status";
          ...
          if ($query2->rowCount() >= $address_count) {
            ...
            $sql = "INSERT INTO management_addresses (unique_id, user_unique_id, firstname, lastname, address, additional_information, city, state, country, added_date, last_modified, status)
            ...
            if ($query->rowCount() > 0) {
              ...
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Not inserted (new management user address)";
            }
          }
