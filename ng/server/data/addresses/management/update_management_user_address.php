    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          $sql2 = "SELECT address FROM management_addresses WHERE unique_id=:unique_id AND user_unique_id=:user_unique_id AND status=:status";
          $query2 = $conn->prepare($sql2);
          $query2->bindParam(":unique_id", $address_unique_id);
          $query2->bindParam(":user_unique_id", $management_user_unique_id);
          $query2->bindParam(":status", $active);
          $query2->execute();

          if ($query2->rowCount() > 0) {

            $sql = "UPDATE management_addresses SET firstname=:firstname, lastname=:lastname, address=:address, additional_information=:additional_information, city=:city, state=:state, country=:country,
            ...
            if ($query->rowCount() > 0) {
              ...
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Not edited (management user address)";
            }
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Management user address not found";
          }
