  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql2 = "SELECT status FROM users_addresses WHERE unique_id=:unique_id AND user_unique_id=:user_unique_id AND status=:status";
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql = "UPDATE users_addresses SET status=:status, last_modified=:last_modified WHERE unique_id=:unique_id AND user_unique_id=:user_unique_id";
          ...
          if ($query->rowCount() > 0) {
            ...
            $returnvalue->engineErrorMessage = "Not removed (user address)";
          }
