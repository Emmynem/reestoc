      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          if ($query2->rowCount() > 0) {
            ...
            $sql3 = "SELECT name FROM brands WHERE (name=:name OR stripped=:stripped) AND unique_id!=:unique_id AND status=:status";
            $query3 = $conn->prepare($sql3);
            $query3->bindParam(":name", $name);
            $query3->bindParam(":stripped", $stripped);
            $query3->bindParam(":unique_id", $brand_unique_id);
            ...
            if ($query3->rowCount() > 0) {
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Brand already exists";
            }
            else {
              $sql = "UPDATE brands SET edit_user_unique_id=:edit_user_unique_id, name=:name, stripped=:stripped, details=:details, last_modified=:last_modified WHERE unique_id=:unique_id";
              ...
              if ($query->rowCount() > 0) {
                ...
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Not edited (brand)";
              }
            }
