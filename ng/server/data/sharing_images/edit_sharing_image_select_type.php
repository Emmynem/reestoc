<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      // $data['sharing_image_unique_id'] = $_POST['sharing_image_unique_id'];
      // $data['user_unique_id'] = $_POST['user_unique_id'];
      // $data['sharing_unique_id'] = $_POST['sharing_unique_id'];
      // $data['upload_type'] = $_POST['upload_type'];

      $sharing_image_unique_id = isset($_POST['sharing_image_unique_id']) ? $_POST['sharing_image_unique_id'] : $data['sharing_image_unique_id'];
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql3 = "SELECT file FROM sharing_images WHERE unique_id=:unique_id AND sharing_unique_id=:sharing_unique_id AND status=:status";
          $query3 = $conn->prepare($sql3);
          $query3->bindParam(":unique_id", $sharing_image_unique_id);
          ...
          if (empty($_FILES['file']['name'])) {
            ...
            $the_sharing_image_details = $query3->fetch();
            $old_sharing_image = $the_sharing_image_details[0];
                              ...
                              if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                ...
                                $targetPath = $path; // Target path where file is to be stored
                                 ...
                                 $file = $path;
                                  ...
                                  $sql = "UPDATE sharing_images SET edit_user_unique_id=:edit_user_unique_id, image=:image, file=:file, file_size=:file_size, last_modified=:last_modified
                                  WHERE unique_id=:unique_id AND sharing_unique_id=:sharing_unique_id";
                                  $query = $conn->prepare($sql);
                                  $query->bindParam(":unique_id", $sharing_image_unique_id);
                                  ...
                                  if($query){
                                    if($count > 1){
                                      unlink($path_to_delete."/".$old_sharing_image);
                                      $returnvalue = new genericClass();
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Old Product image not found";
          }
