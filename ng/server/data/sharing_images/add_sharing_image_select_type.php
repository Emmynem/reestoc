<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $image_count = $functions->check_length_10;
      $image_count_minimum = $functions->check_length_3;
      ...
      // $data['sharing_unique_id'] = $_POST['sharing_unique_id'];
      // $data['upload_type'] = $_POST['upload_type'];

      $user_unique_id = isset($_POST['user_unique_id']) ? $_POST['user_unique_id'] : $data['user_unique_id'];
      $sharing_unique_id = isset($_POST['sharing_unique_id']) ? $_POST['sharing_unique_id'] : $data['sharing_unique_id'];
      $upload_type = isset($_POST['upload_type']) ? $_POST['upload_type'] : $data['upload_type'];

      $path_to_upload = "../../../../images/sharing_images/";
      // $path_to_delete = $_SERVER['DOCUMENT_ROOT']."/images/sharing_images"; // For online own
      $path_to_delete = $_SERVER['DOCUMENT_ROOT']."/cerotics_store/images/sharing_images"; // For offline own
      // $path_to_save = "https://www.images.reestoc.com/images/sharing_images/";
      $path_to_save = "https://www.reestoc.com/images/sharing_images/";
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql3 = "SELECT unique_id FROM sharing_images WHERE sharing_unique_id=:sharing_unique_id AND status=:status";
          $query3 = $conn->prepare($sql3);
          $query3->bindParam(":sharing_unique_id", $sharing_unique_id);
          $query3->bindParam(":status", $active);
          $query3->execute();

          $recent_image_count = $query3->rowCount();

          $files_count = !empty($_FILES['file']['name']) ? count($_FILES['file']['name']) : null;

          $total_image_count = $recent_image_count + $files_count;
          ...
          else if ($files_count < $image_count_minimum) {
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Must be up to 3 or more files";
          }
          else if ($files_count > $image_count) {
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Not more than 10 files allowed";
          }
          else if ($query3->rowCount() >= $image_count) {
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Product images maxed out";
          }
          else if($total_image_count > $image_count){
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "You have ".$recent_image_count." images already, max is 10";
          }
          else {
                              ...
                              if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                ...
                                $targetPath = $path; // Target path where file is to be stored
                                 ...
                                 $file = $path;
                                  ...
                                  $sql = "INSERT INTO sharing_images (unique_id, user_unique_id, edit_user_unique_id, sharing_unique_id, image, file, file_size, added_date, last_modified, status)
                                  VALUES (:unique_id, :user_unique_id, :edit_user_unique_id, :sharing_unique_id, :image, :file, :file_size, :added_date, :last_modified, :status)";
