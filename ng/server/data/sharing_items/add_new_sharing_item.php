    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          $sql2 = "SELECT name, expiration, expiry_date, total_no_of_persons, current_no_of_persons FROM sharing_items WHERE (name=:name OR stripped=:stripped) ORDER BY added_date DESC LIMIT 1";
          ...
          if ($query2->rowCount() > 0) {
            ...
            $sql = "INSERT INTO sharing_items (unique_id, user_unique_id, edit_user_unique_id, name, stripped, description, total_price, split_price, total_no_of_persons, current_no_of_persons,
            expiration, starting_date, expiry_date, completion, added_date, last_modified, status)
            VALUES (:unique_id, :user_unique_id, :edit_user_unique_id, :name, :stripped, :description, :total_price, :split_price, :total_no_of_persons, :current_no_of_persons,
            :expiration, :starting_date, :expiry_date, :completion, :added_date, :last_modified, :status)";
            ...
            $query->bindParam(":current_no_of_persons", $zero);
            ...
            $query->bindParam(":completion", $pending);
            ...
            if ($query->rowCount() > 0) {
              ...
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Not inserted (new sharing item)";
            }
