<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $service = isset($_POST['service']) ? $_POST['service'] : $data['service'];
      $offered_service_category_unique_id = isset($_POST['offered_service_category_unique_id']) ? $_POST['offered_service_category_unique_id'] : $data['offered_service_category_unique_id'];
      $details = isset($_POST['details']) ? $_POST['details'] : $data['details'];
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          $sql3 = "SELECT unique_id FROM offered_services_category WHERE unique_id=:unique_id";
          $query3 = $conn->prepare($sql3);
          $query3->bindParam(":unique_id", $offered_service_category_unique_id);
          $query3->execute();

          $sql2 = "SELECT unique_id FROM offered_services WHERE sub_product_unique_id=:sub_product_unique_id AND service=:service";
          $query2 = $conn->prepare($sql2);
          $query2->bindParam(":sub_product_unique_id", $sub_product_unique_id);
          $query2->bindParam(":service", $service);
          $query2->execute();

          if ($query3->rowCount() < 0) {
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Product service category not found";
          }
                                ...
                                if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                  ...
                                  $targetPath = $path; // Target path where file is to be stored
                                   ...
                                   $file = $path;
                                    ...
                                    $sql = "INSERT INTO offered_services (unique_id, user_unique_id, edit_user_unique_id, sub_product_unique_id, service, offered_service_category_unique_id, details, price, image, file, file_size, added_date, last_modified, status)
                                    VALUES (:unique_id, :user_unique_id, :edit_user_unique_id, :sub_product_unique_id, :service, :offered_service_category_unique_id, :details, :price, :image, :file, :file_size, :added_date, :last_modified, :status)";
