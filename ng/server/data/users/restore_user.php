  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql2 = "SELECT status FROM users WHERE unique_id=:unique_id AND status=:status";
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql = "UPDATE users SET status=:status, last_modified=:last_modified WHERE unique_id=:unique_id";
          ...
          if ($query->rowCount() > 0) {
            ...
            $returnvalue->engineErrorMessage = "Not restored (user)";
          }
