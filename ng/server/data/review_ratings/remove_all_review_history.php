  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql2 = "SELECT unique_id, product_unique_id, rating FROM review_history WHERE user_unique_id=:user_unique_id AND status=:status";
        ...
        if ($query2->rowCount() > 0) {
          foreach ($result as $key => $value) {
            ...
            $current_review_history_unique_id = $value['unique_id'];
            $current_review_history_product_unique_id = $value['product_unique_id'];
