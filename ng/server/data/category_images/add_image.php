<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      // $data['the_category_unique_id'] = $_POST['the_category_unique_id'];
      // $data['upload_type'] = $_POST['upload_type'];

      $user_unique_id = isset($_POST['user_unique_id']) ? $_POST['user_unique_id'] : $data['user_unique_id'];
      $the_category_unique_id = isset($_POST['the_category_unique_id']) ? $_POST['the_category_unique_id'] : $data['the_category_unique_id'];
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql3 = "SELECT unique_id FROM category_images WHERE the_category_unique_id=:the_category_unique_id AND status=:status";
        ...
        if (empty($_FILES['file']['name'])) {
          ...
          $returnvalue = new genericClass();
          $returnvalue->engineError = 2;
          $returnvalue->engineErrorMessage = "This category images maxed out";
        }
        // else if($total_image_count > $image_count){
        //   $returnvalue = new genericClass();
        //   $returnvalue->engineError = 2;
        //   $returnvalue->engineErrorMessage = "You have ".$recent_image_count." images already, max is 1";
        // }
        else {
                            ...
                            if((isset($newFilename)) && ($newFilename != "Image already exist")){
                              ...
                              if(compressImage($sourcePath, $path, 60) == 1){
                               $file = $path;
                                ...
                                $sql = "INSERT INTO category_images (unique_id, user_unique_id, edit_user_unique_id, the_category_unique_id, image, file, file_size, added_date, last_modified, status)
                                VALUES (:unique_id, :user_unique_id, :edit_user_unique_id, :the_category_unique_id, :image, :file, :file_size, :added_date, :last_modified, :status)";
