    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if (in_array($price,$not_allowed_values) && $price != 0) {
          ...
          $sql2 = "SELECT unique_id FROM pickup_locations WHERE unique_id=:unique_id AND status=:status";
          ...
          if ($query2->rowCount() > 0) {
            ...
            $sql = "UPDATE pickup_locations SET edit_user_unique_id=:edit_user_unique_id, price=:price, last_modified=:last_modified
            WHERE unique_id=:unique_id AND default_pickup_location_unique_id=:default_pickup_location_unique_id AND sharing_unique_id=:sharing_unique_id";
            ...
            if ($query->rowCount() > 0) {
              ...
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Not edited (sharing item pickup location shipping fee)";
            }
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Sharing Item Pickup Location Shipping fee not found";
          }
