<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $sharing_unique_ids = isset($_GET['sharing_unique_ids']) ? $_GET['sharing_unique_ids'] : $data['sharing_unique_ids'];
        ...
        if (in_array($price,$not_allowed_values) && $price != 0) {
          ...
          foreach ($default_pickup_location_unique_ids as $key => $default_pickup_location_unique_id){
            ...
            if ($query3->rowCount() > 0) {
              ...
              if (!is_array($sharing_unique_ids)) {
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Sharing IDs is not an array";
              }
              else if (in_array($sharing_unique_ids,$not_allowed_values)) {
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Sharing IDs is required";
              }
              else {

                foreach ($sharing_unique_ids as $key => $sharing_unique_id){

                  $sql2 = "SELECT unique_id FROM pickup_locations WHERE sharing_unique_id=:sharing_unique_id AND default_pickup_location_unique_id=:default_pickup_location_unique_id";
                  ...
                  if ($query2->rowCount() > 0) {
                    if (count($sharing_unique_ids) <= 1) {
                      $returnvalue = new genericClass();
                      $returnvalue->engineError = 2;
                      $returnvalue->engineErrorMessage = "Sub Product pickup location already exists";
                    }
                    else {
                      $returnvalue = new genericClass();
                    ...
                    if ($query->rowCount() > 0) {
                      ...
                      $returnvalue = new genericClass();
                      $returnvalue->engineError = 2;
                      $returnvalue->engineErrorMessage = $key." index, Not inserted (new sharing item pickup location)";
                    }
                  }
