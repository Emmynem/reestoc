<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $rider_user_unique_id = isset($_GET['user_unique_id']) ? $_GET['user_unique_id'] : $data['user_unique_id'];
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $validation = $functions->update_management_user_role_and_access_validation($access);
        ...
        if ($validation["error"] == true) {
          ...
          $sql2 = "SELECT fullname FROM riders WHERE unique_id=:unique_id AND status=:status";
          $query2 = $conn->prepare($sql2);
          $query2->bindParam(":unique_id", $rider_user_unique_id);
          $query2->bindParam(":status", $active);
          $query2->execute();

          if ($query2->rowCount() > 0) {

            $sql = "UPDATE riders SET edit_user_unique_id=:edit_user_unique_id, access=:access, last_modified=:last_modified WHERE unique_id=:unique_id";
            $query = $conn->prepare($sql);
            $query->bindParam(":edit_user_unique_id", $edit_user_unique_id);
            $query->bindParam(":access", $access);
            $query->bindParam(":unique_id", $rider_user_unique_id);
            ...
            if ($query->rowCount() > 0) {
              ...
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Not edited (rider access)";
            }
