      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          if ($query2->rowCount() > 0) {
            ...
            $sql3 = "SELECT name FROM mini_category WHERE (name=:name OR stripped=:stripped) AND ((category_unique_id=:category_unique_id OR category_unique_id IS NULL) AND (sub_category_unique_id=:sub_category_unique_id OR sub_category_unique_id IS NULL) AND status=:status)";
            ...
            if ($query3->rowCount() > 0) {
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Mini Category already exists";
            }
            else {
              $sql = "UPDATE mini_category SET edit_user_unique_id=:edit_user_unique_id, name=:name, stripped=:stripped, last_modified=:last_modified WHERE unique_id=:unique_id AND (category_unique_id=:category_unique_id OR category_unique_id IS NULL) AND (sub_category_unique_id=:sub_category_unique_id OR sub_category_unique_id IS NULL)";
              ...
              if ($query->rowCount() > 0) {
                ...
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Not edited (mini category)";
              }
            }
