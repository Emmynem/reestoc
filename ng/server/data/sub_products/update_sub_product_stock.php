  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $validation = $functions->update_product_stock_validation($stock);
        ...
        if ($validation["error"] == true) {
          ...
          $sql2 = "SELECT stock_remaining FROM products WHERE unique_id=:unique_id AND product_unique_id=:product_unique_id AND status=:status";
          ...
          if ($query2->rowCount() > 0) {
            ...
            $stock_details = $query2->fetch();
            $stock_remaining = (int)$stock_details[0];

            $sql = "UPDATE products SET edit_user_unique_id=:edit_user_unique_id, stock=:stock, stock_remaining=:stock_remaining, last_modified=:last_modified WHERE unique_id=:unique_id";
            ...
            if ($query->rowCount() > 0) {
              ...
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Not edited (sub product stock)";
            }
