  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $validation = $functions->coupon_validation($user_unique_id, $sub_product_unique_id, $code, $name, $percentage, $total_count, $expiry_date);
        ...
        if ($validation["error"] == true) {
          ...
          if (!in_array($user_unique_id,$not_allowed_values)) {
            $sql2 = "SELECT name FROM coupons WHERE user_unique_id=:user_unique_id AND expiry_date >:today AND status=:status";
            ...
            if ($query2->rowCount() > 0) {
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Current user coupon hasn't expired yet";
            }
            else {
            ...
            if ($query2->rowCount() > 0) {
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Current product coupon hasn't expired yet";
            }
            else {
