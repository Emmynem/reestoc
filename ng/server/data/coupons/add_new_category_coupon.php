      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          if ($query4->rowCount() > 0) {
            ...
            $sql2 = "SELECT name FROM coupons WHERE mini_category_unique_id=:mini_category_unique_id AND code=:code AND expiry_date >:today AND status=:status";
            ...
            if ($query2->rowCount() > 0) {
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Current mini category coupon hasn't expired yet";
            }
            else {
              ...
              if ($query->rowCount() > 0) {
                ...
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Not inserted (new mini category coupon)";
              }
            }
