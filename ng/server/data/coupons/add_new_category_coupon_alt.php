    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          $sql4 = "SELECT unique_id FROM products WHERE category_unique_id=:category_unique_id AND sub_category_unique_id=:sub_category_unique_id AND mini_category_unique_id=:mini_category_unique_id AND status=:status";
          ...
          if ($query4->rowCount() > 0) {
            $the_product_unique_ids = $query4->fetchAll();

            foreach ($the_product_unique_ids as $key => $value_product_unique_id){

              $product_unique_id = $value_product_unique_id['unique_id'];

              $sql5 = "SELECT unique_id FROM sub_products WHERE product_unique_id=:product_unique_id AND status=:status";
              ...
              if ($query5->rowCount() > 0) {
                $the_sub_product_unique_ids = $query5->fetchAll();
                ...
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Sub products not found ".$product_unique_id;
              }
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Products not found";
          }
