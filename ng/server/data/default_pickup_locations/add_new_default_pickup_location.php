    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($validation["error"] == true) {
          ...
          $sql2 = "SELECT address FROM default_pickup_locations WHERE firstname=:firstname AND lastname=:lastname AND city=:city AND state=:state AND country=:country AND status=:status";
          ...
          if ($query2->rowCount() > 0) {
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Pickup Location already exists";
          }
          else {
            $unique_id = $functions->random_str(20);

            $sql = "INSERT INTO default_pickup_locations (unique_id, user_unique_id, edit_user_unique_id, firstname, lastname, address, additional_information, city, state, country, added_date, last_modified, status)
            VALUES (:unique_id, :user_unique_id, :edit_user_unique_id, :firstname, :lastname, :address, :additional_information, :city, :state, :country, :added_date, :last_modified, :status)";
            ...
            if ($query->rowCount() > 0) {
              ...
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Not inserted (new default pickup location)";
            }
          }
