  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql2 = "SELECT status FROM default_pickup_locations WHERE unique_id=:unique_id AND status=:status";
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql = "UPDATE default_pickup_locations SET edit_user_unique_id=:edit_user_unique_id, status=:status, last_modified=:last_modified WHERE unique_id=:unique_id";
          ...
          if ($query->rowCount() > 0) {
            ...
            $sql = "UPDATE pickup_locations SET edit_user_unique_id=:edit_user_unique_id, status=:status, last_modified=:last_modified WHERE default_pickup_location_unique_id=:default_pickup_location_unique_id";
            $query = $conn->prepare($sql);
            $query->bindParam(":edit_user_unique_id", $user_unique_id);
            ...
            if ($query->rowCount() > 0) {
              ...
              // $returnvalue->engineErrorMessage = "Not removed (pickup locations)";
            }
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Not removed (default pickup locations)";
          }
