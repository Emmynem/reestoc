<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $null = $functions->null;
      $completion_status = $functions->add_shipping_fee;
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $sql3 = "SELECT price, location FROM shipping_fees WHERE unique_id=:unique_id AND sub_product_unique_id=:sub_product_unique_id";
        $query3 = $conn->prepare($sql3);
        $query3->bindParam(":unique_id", $shipping_fee_unique_id);
        $query3->bindParam(":sub_product_unique_id", $sub_product_unique_id);
        $query3->execute();

        if ($query3->rowCount() > 0) {
          $the_shipping_fee_price_details = $query3->fetch();
          $the_shipping_fee_price = (int)$the_shipping_fee_price_details[0];
          $the_shipping_fee_location = $the_shipping_fee_price_details[1];

          $sql2 = "UPDATE orders SET shipping_fee_unique_id=:shipping_fee_unique_id, last_modified=:last_modified WHERE unique_id=:unique_id AND user_unique_id=:user_unique_id AND tracker_unique_id=:tracker_unique_id";
          $query2 = $conn->prepare($sql2);
          $query2->bindParam(":shipping_fee_unique_id", $shipping_fee_unique_id);
          ...
          $returnvalue = new genericClass();
          $returnvalue->engineError = 2;
          $returnvalue->engineErrorMessage = "Shipping fee invalid (not found)";
        }
